...
Den Schauplatz \textbf{Finsterkoppen} und die \textbf{Finsterkopp-Binge} waren schon im Computerspiel \emph{\href{https://de.wiki-aventurica.de/wiki/Sternenschweif_(1994)}{Sternenschweif}} (Attic, 1994) zu erkunden. %, das ich vor etwa 20 Jahren das letzte Mal gespielt habe.
...
Die Idee von \textbf{Rattengold} als überschwerem 13.\,Unmetall haben Simon Schollenberger, Stefan Weber und Stefan Löffler für das Wettbewerbs-Abenteuer \enquote{\link{https://www.orkenspalter.de/filebase/index.php?file/53-totgeboren/}{Totgeboren}} entwickelt.
...
\item Nehmen Sie sich Zeit, Finsterkoppen zu erkunden -- nutzen Sie die Ortsbeschreibung im Anhang (\pageref{hdb_anhang_finsterkoppen}). Der \textbf{Tiefe Rat} kommt erst in einigen Tagen zusammen, zuvor kann die Gruppe gemeinsam ein zünftiges Pilzbier bei \textbf{Schwarzbart} trinken,
...
\begin{center}
\vspace{-0.4cm}
\includegraphics[width=0.64\linewidth]{Ingerimm_distressed}
\vspace{-0.5cm}
\end{center}
...
Die Versammlungshalle verbindet das \textbf{große Tor} mit dem \textbf{Angroschtempel}. Vier weitere Gänge führen seitlich tiefer in den Berg.
...
\vorlesen{%
Im gesamten Tempel ist es \emph{heiß} (\ilaris{35}), da stets mehrere Feuer und Öfen brennen.
...
Die Wände sind mit Angram-Runen bedeckt, die die Geschichte der Kinder Angroschs im Allgemeinen und des \fkvs im Besonderen im Lichte des Angrosch-Glaubens auslegen.\\
...
Die Hitze aus der Tiefe ist in zwei Schritt Umkreis deutlich spürbar (\emph{Kh\^omglut}, \ilaris{35}).
...
bei empfänglichen (Vorteil: Prophezeien, \ilaris{30}) zudem Visionen auslösen können.
\\
...
Ein stets verschlossenes \textbf{goldenes Portal} führt von hier aus in den aufgegebenen Teil der \textbf{Finsterkopp-Binge}.
...
\enquote{%
	Geschätzter König \textbf{Gerambolosch}, hochverehrte Mütterchen und Väterchen: Angrosch ist mein Zeuge, viele Fragen treiben den Tiefen Rat heute um.
...
\vorlesen{%
\enquote{Hochverehrter Vater \textbf{Xermosch}, Eile mit Weile und alles, wie es Angrosch gefügt! Wie Ihr selbst sagt, sind es Fremde.
...
\enquote{Wohl wahr, bei Angrosch}, sagt Inradon \textbf{Xermosch} schließlich langsam. Wie um Zeit zu gewinnen, erklärt er die Gesetze, die den meisten Anwesenden klar sind:
...
\enquote{Erst danach können andere Kandidaturen erklärt werden und die Wahl erfolgt durch den Tiefen Rat.}
}
...
\vorlesen{%
 ...
 Er bricht leicht verzweifelt ab. Dann fast er seinen Mut zusammen und wendet sich an \textbf{Gerambolosch}:
...
der ihm etwas \textbf{Rattengold} beigibt und dass dieses Material in der \textbf{Finsterkopp-Binge} zu finden ist.
}
...
misserfolg={	\textbf{Bonderik} wird Inradon \textbf{Xermosch} -- der nicht selbst auf die Idee kommt -- bitten, die \textbf{Angram}-Runen zu studieren.
	...
	Tatsächlich hält der Inradon diese Möglichkeit jedoch für eher theoretisch und wird sie nicht erwähnen, wenn er dem Rat das Ergebnis seiner Nachforschung vorstellt.},
...
\begin{center}
\bild[0.8\textwidth]{Zwergenrat.png}
\end{center}
...
{Rededuell (\ilaris{55})}{Für Menschen wird \emph{ungewohnte Umgebung} angenommen.
...
\info{\textbf{Ziel der Gruppe:} Das Szepter finden und Hinweise auf den Saboteur erhalten.
	...
	\textbf{Vorschläge für längeres Spiel:}
	\begin{enumerate}
		\item Durch die Gänge der Wühlschrate sind neben der Riesenamöbe auch \link{https://ilaris-online.de/app/kreatur/92}{Höhlenspinnen} (\ilaris{101}) und \link{https://ilaris-online.de/app/kreatur/81}{Gruftasseln} eingedrungen.
...
\smallskip
	...
	\item Verzichten Sie auf die Begegnung mit dem Bosnickel und seien sie stattdessen großzügig mit den Informationen beim Auffinden des \textbf{Rattengolds}.
\end{enumerate}
}
\vorlesen{
...
Vereinzelt hat sich Phosphorpilz ausgebreitet, so dass Sternenlicht (\ilaris{38}) herrscht.
...
Die gesamte Binge gilt als enger Raum (\ilaris{38}).
...
\end{center}
Hat jemand in der Gruppe den Vorteil Zwergennase (\ilaris{30}) oder Gefahreninstinkt
...
vergleichend=nein,
pw=24,
erfolg={\vorlesen{Ihr findet bald die Platten im Boden der Gangmitte, die ein wenig herausstehen. Vorsichtig übersteigt ihr sie. In der gegenüberliegenden Wand entdeckt ihr schließlich drei in Unebenheiten des Steins versteckte Löcher.}
...
ist für \emph{Zwergennase} oder \emph{Gefahreninstinkt} jeweils -4 leichter.}
...
In einer davon liegt eine seit Jahrhunderten gespannte Bärenfalle (\ilaris{63}).
Wer nach Beute sucht und eine \emph{Sinnenschärfe-Probe (III, 24, -4} durch jede Stufe \emph{Angepasst (Dunkelheit)}) misslingt, schnappt die Falle zu und Wundfieber (\ilaris{35}) droht.

Anschließend oder bei gelungener Probe wird ein verzauberter \textbf{Asthenil}-Ring gefunden (\ilaris{78}: \emph{ladungsbasiert, aufladbar}, eine Ladung, \biglittlecap{Elementarbann} mit \emph{Modifikation Gegenzauber} (\ilaris{126}), \emph{Erfolgswert} 20).
...
Sie wäre als \emph{archaisches Labor} (\ilaris{64}) benutzbar.
...
Es muss sich um \textbf{Rattengold} handeln, eines der sogenannten 13 Unmetalle.\\
	\textbf{20:} \textbf{Rattengold} wird dem \textbf{Namenlosen} zugeordnet. Es heißt, dass Gefolgsleute dieses Gottes nicht mit dem hohen Gewicht zu kämpfen haben.\\
	...
	\textbf{28:} \textbf{Rattengold} lässt sich für einige \textbf{Alchemika} verwenden, beispielsweise als potentes Substitut für Gold und Bernstein im \textbf{Bannstaub} (\ilaris{64}) oder für Zinnober im \textbf{Purpurblitz} (\ilaris{66}).\\
	\textbf{32:} Es heißt, dass der \textbf{Kaisertöter} -- eine mythische Klinge, mit der der Kult des Namenlosen Herrscherhäuser stürzt -- aus \textbf{Rattengold} geschmiedet wird.}
\newpage
\spaltenanfang
...
Unter ihnen ist niemand über einen Schritt groß -- die Löcher sind so klein, dass Spielfiguren ihnen nicht folgen können.\bigskip
...
Ihre Versuche, die Gruppe zu behindern, sind eher Streiche.} 
...
Neben der \textbf{Bärenfalle} (\ilaris{63}) sollten Sie auch kreativere Lösungen ermöglichen und belohnen.
...
\info{
Geben Sie Ihrer Gruppe nun das Rätsel auf S.\,\pageref{raetsel} (Lizenzhinweise auf S.\,\pageref{lizenz}).
Ich empfehle, es auf zwei verschiedenen Seite auszudrucken, auszuschneiden und passgenau aufeinander zu kleben -- doppelseitiger Ausdruck gibt leider ein verschobenes Ergebnis. Legen Sie das resultierende Blatt möglichst flach auf den Tisch. Die Entdeckung der Rückseite soll schon den ersten Aha-Effekt darstellen:}
...
misserfolg={Wird der Hebel gezogen, während eine falsche Kombination eingestellt ist, klappt die Brücke weg und die Person am Hebel stürzt unweigerlich in die Tiefe.
	Dies könnte leicht den Tod im Magma bedeuten, so dass sie sich glücklich schätzen muss, wenn scharfkantige Obsidianfelsen sie aufhalten.
	4W6+4 \emph{Sturz}schaden verursachen diese dennoch; nach einer \emph{Akrobatik-Probe} (16) die Hälfte (\ilaris{34}).
	...
	Das Seil erleidet bei jedem Sturz 2W6~SP.},
]
{Klugheit}{\mbox}
...
\\
An den Wänden erinnern Platten an große Angehörige des \fkvs und ihre wichtigsten Taten.
...
Eine \link{https://ilaris-online.de/app/kreatur/175}{Riesenamöbe} (\ilaris{105}) muss durch ein von den Wühlschraten gegrabenes Loch ihren Weg in die Gruft gefunden haben.
...
Nach dem Kampf kann die Gruppe das Szepter bergen. Es besteht hauptsächlich aus \textbf{Asthenil}, mit Applikationen aus \textbf{Toschkril} und \textbf{Zwergengold}.
Ein großer \textbf{Serpentin} ist an zentraler Stelle eingebettet.
\\
Auch einige Waffen und Werkzeuge finden sich in den amorphen Überresten der Amöbe. Diese sind natürlich von hervorragender Qualität (3xHQ und mehr, \emph{gutes Werkzeug} (\ilaris{61})), waren hier aber eigentlich zur ewigen Ruhe gebettet. Etwas mitzunehmen könnten Angrosch und auch Boron als Frevel auffassen \dots
\vspace{1cm}
\begin{center}
\includegraphics[width=0.45\linewidth]{Lindwurmschlägl}
\end{center}

\columnbreak
\subsubsection{Immer diese Schrate!}
\begin{center}
	\zeichnung[0.8\linewidth]{Wuehlschrat_distressed.png}
\end{center}
...
Nun kann eine \emph{Verfolgungsjagd} (\ilaris{57}, DG\,3) beginnen:\\
...
Selbst in prunkvollen Räumen wie der Versammlungshalle oder dem Angroschtempel herrscht \emph{Dämmerung} (\ilaris{38}).
...
Hier eine Übersichts-Tabelle, mit welchen Eigenheiten Sie den Figuren in welcher Szene \emph{das Leben schwer machen} (\ilaris{15}) können, um Schicksalspunkte zu verteilen:
